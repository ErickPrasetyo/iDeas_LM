...
type
  TSupplierFrm = class(TForm)
    ...
    SCButton1: TSCButton;
    chktext: TcxCheckBox;
    cbAktif: TcxDBCheckBox;
    Masteris_aktif: TStringField;
...
begin
  ...
  try
    DM.vSysDate.Close;
    DM.vSysDate.Open;
    SupplierFrm.Master.Close;
    SupplierFrm.Master.Open;
  except
 ...
 // KaryawanFrm.pnlBottom.Visible:= False;
  SupplierFrm.lblHeader1.Caption:= pNamaMenu;
  SupplierFrm.vtag:= ptag;
  SupplierFrm.UpdateView;
  SupplierFrm.Show;
end;
...
procedure TSupplierFrm.MasterNewRecord(DataSet: TDataSet);
begin
...
//  Mastercontact.AsString:= '-';
  ...
  Masterjenis.AsString:= 'SUPPLIER';
  Masteris_pkp.AsString:= '0'
  
end;

procedure TSupplierFrm.FormCloseQuery(Sender: TObject;
...
procedure TSupplierFrm.MasterBeforePost(DataSet: TDataSet);
begin
...
//  if Masterplafon_hutang.IsNull or (Trim(Masterplafon_hutang.AsString)='') then
//    raise Exception.Create('Plafon Hutang Harus Diisi !')
//  else
...
procedure TSupplierFrm.actCloseExecute(Sender: TObject);
begin
  close;
end;

procedure TSupplierFrm.actAddExecute(Sender: TObject);
begin
  try
...
procedure TSupplierFrm.actEditExecute(Sender: TObject);
begin
  try
    pgMaster.ActivePageIndex:= 1;
    Master.Edit;
...
procedure TSupplierFrm.actDeleteExecute(Sender: TObject);
begin
...
procedure TSupplierFrm.actRefreshExecute(Sender: TObject);
begin
  try
    Master.Close;
    Master.Open
...
procedure TSupplierFrm.actSaveExecute(Sender: TObject);
begin
  try
    Master.Post;
  except
...
procedure TSupplierFrm.actCancelExecute(Sender: TObject);
begin
  try
    Master.Cancel;
  except
...
procedure TSupplierFrm.UpdateView;
...
procedure TSupplierFrm.pgMasterPageChanging(Sender: TObject;
  NewPage: TcxTabSheet; var AllowChange: Boolean);
begin
  AllowChange:= Master.State=dsBrowse
end;

procedure TSupplierFrm.grddbtvMasterDblClick(Sender: TObject);
begin
  pgMaster.ActivePageIndex := 1;
end;
...
procedure TSupplierFrm.cbFieldNamePropertiesChange(Sender: TObject);
begin
...
procedure TSupplierFrm.actSearchExecute(Sender: TObject);
begin
   pnlSearch.Visible:= not pnlSearch.Visible;
end;

procedure TSupplierFrm.EditKeyPress(Sender: TObject;
  var Key: Char);
begin
    if Key=#13 then
...
procedure TSupplierFrm.SCButton1Click(Sender: TObject);
var
  List: TStringList;
        ...
        begin
          ...
          begin
            ...
            if chktext.Checked then
               S := S + '"' + Master.Fields[I].AsString + '"'
            else S := S + Master.Fields[I].AsString;
          end;
          List.Add(S);
          Master.Next;
        end;
      finally
        List.SaveToFile(OpenDialog.FileName);
        List.Free;
      end;
...
procedure TSupplierFrm.btnExportClick(Sender: TObject);
var
excel : Variant;
appPath, ttl :string;
begin
