...
uses
  ...
  kbmMemTable, wwDialog, wwidlg, ZSequence, AdvPanel, AdvAppStyler,
  cxGridBandedTableView, cxGridTableView, AdvToolBar, AdvToolBarStylers,
  ZAbstractRODataset, ZAbstractDataset, AdvReflectionImage, StdCtrls,
  ExtCtrls, DBCtrls, RzDBNav, cxMemo, cxDBEdit, cxCheckListBox,
  cxDBCheckListBox, cxButtonEdit, cxDropDownEdit, cxCalendar, RzPanel,
  SCStdControls, wwdbdatetimepicker, cxContainer, cxTextEdit, cxMaskEdit,
  Wwkeycb, SCControl, cxGridLevel, cxGridCustomTableView,
...
type
  ...
  TKasbonFrm = class(TForm)
    ...
    grdMasterDBTablefullname: TcxGridDBColumn;
    SqKasbon: TZSequence;
    ...
    procedure LblCekOutClick(Sender: TObject);
...
begin
  try
    KasbonFrm := TKasbonFrm.Create(Application);
    with KasbonFrm do begin
...
function TKasbonFrm.CountOutStanding(nK: string): Single;
var s : String;
begin
 Result:= 0;
 try
...
function TKasbonFrm.GetRekGL(param: string): String;
begin

end;

function TKasbonFrm.CheckEditRight(var msg: string): Boolean;
begin
  Result:= True;
  msg:= '';
...
function TKasbonFrm.CheckDeleteRight(var msg: string): Boolean;
begin
  Result:= True;
  msg:= '';
...
procedure TKasbonFrm.UpdateView;
...
begin
  if DBMode=dmNone then begin
    ...
    if DM.isSuperUser then begin
       ...
       edtNK.Properties.Buttons[0].Enabled:= not isBrowse;
    end
    else begin
       edtNK.Properties.ReadOnly:= True;
       edtNK.Properties.Buttons[0].Enabled:= False;
    end;
    ...
    edtTunggakan.Properties.ReadOnly:= True;
...
//    edtRates.Properties.ReadOnly:= isBrowse;
    ...
    LblCekOut.Enabled:= not isBrowse;
  end;
end;

procedure TKasbonFrm.OpenDataset;
begin
  try
    dm.L_Pegawai.Close;
    dm.L_Pegawai.Open;
...
procedure TKasbonFrm.edtNKPropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
...
procedure TKasbonFrm.btnAddClick(Sender: TObject);
begin
...
procedure TKasbonFrm.btnEditClick(Sender: TObject);
begin
  if  pgcMaster.ActivePageIndex=0 then
      pgcMaster.ActivePageIndex:=1;
...
procedure TKasbonFrm.btnDeleteClick(Sender: TObject);
begin
  if  pgcMaster.ActivePageIndex=0 then
      pgcMaster.ActivePageIndex:=1;
...
procedure TKasbonFrm.btnRefreshClick(Sender: TObject);
begin
   OpenDataset;
end;

procedure TKasbonFrm.pgcMasterPageChanging(Sender: TObject;
  NewPage: TcxTabSheet; var AllowChange: Boolean);
begin
  AllowChange:= (DBMode=dmNone) or (DBMode=dmBrowse)
end;

procedure TKasbonFrm.grdMasterDBTableCellDblClick(
...
procedure TKasbonFrm.btnCloseClick(Sender: TObject);
begin
   Close;
end;

procedure TKasbonFrm.FormCloseQuery(Sender: TObject;
...
procedure TKasbonFrm.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action:=caFree;
end;

procedure TKasbonFrm.btnSaveClick(Sender: TObject);
begin
...
//   else Masterid_rek_gl_debet.AsString:= DM.GetRekForm(vjns_transaksi, 'D');
  ...
  if Mastertunggakan.AsFloat=0 then begin
     LblCekOutClick(nil);
  end;
...
procedure TKasbonFrm.btnCancelClick(Sender: TObject);
begin
...
procedure TKasbonFrm.MasterBeforePost(DataSet: TDataSet);
begin
   ...
   if Masternk.IsNull or (Trim(Masternk.AsString)='') then
      raise Exception.Create('NO. PEGAWAI Harus diisi!')
   else
...
procedure TKasbonFrm.MasterNewRecord(DataSet: TDataSet);
begin
...
procedure TKasbonFrm.LblL_ApprovedClick(Sender: TObject);
begin
   try
       DM.LApprover.Close;
...
procedure TKasbonFrm.btnPrintClick(Sender: TObject);
var i : integer;
begin
 if qBrowse.RecordCount=0 then
    Exit;
       ...
       if RGModeCetak.ItemIndex=1 then begin
        ...
        try
           ...
           kmtMaster.Open;
           kmtMaster.Append;
           kmtMasterlogo.LoadFromFile(ExtractFilePath(Application.ExeName)+'\IMAGES\LOGO.JPG');
           kmtMasternk.AsString:=Masternk.AsString;
           kmtMasternama_pegawai.AsString:= Masternk.AsString+'/'+ Masternama_pegawai.AsString;
...
procedure TKasbonFrm.MasterBeforeOpen(DataSet: TDataSet);
begin
  Master.Params.ParamByName('id_kasbon').Value:= qBrowseid_kasbon.AsLargeInt;
end;

procedure TKasbonFrm.MasterBeforeEdit(DataSet: TDataSet);
var msg: string;
begin
...
procedure TKasbonFrm.MasterBeforeDelete(DataSet: TDataSet);
var msg: string;
begin
  if Master.RecordCount=0 then
    Abort;
...
procedure TKasbonFrm.qBrowseBeforeOpen(DataSet: TDataSet);
begin
  ...
  if not DM.isSuperUser then
     qBrowse.Params.ParamByName('vuser_conn').AsString:= DM.UserConnect
  else
     qBrowse.Params.ParamByName('vuser_conn').AsString:= '%';
end;

procedure TKasbonFrm.btnOKClick(Sender: TObject);
begin
   try
     qBrowse.Close;
     qBrowse.Open;
...
procedure TKasbonFrm.dtpStartCloseUp(Sender: TObject);
var dt: TDateTime;
    y,m,d: Word;
begin
...
procedure TKasbonFrm.dtpEndCloseUp(Sender: TObject);
var dt: TDateTime;
    y,m,d: Word;
begin
...
procedure TKasbonFrm.tsDetailShow(Sender: TObject);
begin
   try
     Master.Close;
     Master.Open;
...
procedure TKasbonFrm.EditKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TKasbonFrm.dbnMasterClick(Sender: TObject; Button: TNavigateBtn);
begin
   try
     Master.Close;
     Master.Open;
...
procedure TKasbonFrm.LblDisetujuiClick(Sender: TObject);
begin
  try
    DM.LReviewer.Close;
...
procedure TKasbonFrm.LblCekOutClick(Sender: TObject);
begin
  Mastertunggakan.AsFloat:= CountOutStanding(Trim(edtNK.Text));
end;

procedure TKasbonFrm.F4KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
   edtNKPropertiesButtonClick(nil,0);
end;

procedure TKasbonFrm.edtRekGLPropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
begin
    if (DBMode=dmNone) or (DBMode=dmBrowse) then
      Exit;
...
procedure TKasbonFrm.F6KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
