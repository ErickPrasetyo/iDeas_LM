object CustomerFrm: TCustomerFrm
  Left = 342
  Top = 71
  Width = 924
  Height = 602
  Caption = 'Master Customer'
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  FormStyle = fsMDIChild
  OldCreateOrder = False
  Position = poDefault
  Visible = True
  WindowState = wsMaximized
  OnClose = FormClose
  OnCloseQuery = FormCloseQuery
  OnCreate = FormCreate
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object pnlHeader: TAdvPanel
    Left = 0
    Top = 0
    Width = 908
    Height = 81
    Align = alTop
    BevelOuter = bvNone
    Color = 13616833
    Font.Charset = DEFAULT_CHARSET
    Font.Color = 7485192
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    ParentFont = False
    TabOrder = 0
    UseDockManager = True
    Version = '2.3.0.7'
    AutoHideChildren = False
    BorderColor = 16765615
    Caption.Color = 12105910
    Caption.ColorTo = 10526878
    Caption.Font.Charset = DEFAULT_CHARSET
    Caption.Font.Color = clWhite
    Caption.Font.Height = -11
    Caption.Font.Name = 'MS Sans Serif'
    Caption.Font.Style = []
    Caption.GradientDirection = gdVertical
    Caption.Indent = 2
    Caption.ShadeLight = 255
    CollapsColor = clNone
    CollapsDelay = 0
    ColorTo = 12958644
    ColorMirror = 12958644
    ColorMirrorTo = 15527141
    ShadowColor = clBlack
    ShadowOffset = 0
    StatusBar.BorderStyle = bsSingle
    StatusBar.Font.Charset = DEFAULT_CHARSET
    StatusBar.Font.Color = clWhite
    StatusBar.Font.Height = -11
    StatusBar.Font.Name = 'Tahoma'
    StatusBar.Font.Style = []
    StatusBar.Color = 10592158
    StatusBar.ColorTo = 5459275
    StatusBar.GradientDirection = gdVertical
    Styler = APS
    FullHeight = 41
    object ariHeader: TAdvReflectionImage
      Left = 15
      Top = 10
      Width = 56
      Height = 71
      AutoSize = False
      Picture.Data = {
        
        6082}
      Version = '1.5.0.2'
    end
    object lblHeader2: TLabel
      Left = 79
      Top = 32
      Width = 262
      Height = 42
      AutoSize = False
      Caption = 
        'Form ini berisi daftar customer yang menjadi customer perusahaan' +
        ' '
      Color = clWhite
      Font.Charset = ANSI_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentColor = False
      ParentFont = False
      Transparent = True
      WordWrap = True
    end
    object lblHeader1: TLabel
      Left = 69
      Top = 7
      Width = 183
      Height = 23
      Caption = 'Master Customer'
      Color = clWhite
      Font.Charset = ANSI_CHARSET
      Font.Color = 10901530
      Font.Height = -19
      Font.Name = 'Verdana'
      Font.Style = [fsBold]
      ParentColor = False
      ParentFont = False
      Transparent = True
    end
  end
  object pnlNavigator: TAdvPanel
    Left = 0
    Top = 523
    Width = 908
    Height = 40
    Align = alBottom
    BevelOuter = bvNone
    Color = 13616833
    Font.Charset = DEFAULT_CHARSET
    Font.Color = 7485192
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    ParentFont = False
    TabOrder = 1
    UseDockManager = True
    Version = '2.3.0.7'
    AutoHideChildren = False
    BorderColor = 16765615
    Caption.Color = 12105910
    Caption.ColorTo = 10526878
    Caption.Font.Charset = DEFAULT_CHARSET
    Caption.Font.Color = clWhite
    Caption.Font.Height = -11
    Caption.Font.Name = 'MS Sans Serif'
    Caption.Font.Style = []
    Caption.GradientDirection = gdVertical
    Caption.Indent = 2
    Caption.ShadeLight = 255
    CollapsColor = clNone
    CollapsDelay = 0
    ColorTo = 12958644
    ColorMirror = 12958644
    ColorMirrorTo = 15527141
    ShadowColor = clBlack
    ShadowOffset = 0
    StatusBar.BorderStyle = bsSingle
    StatusBar.Font.Charset = DEFAULT_CHARSET
    StatusBar.Font.Color = clWhite
    StatusBar.Font.Height = -11
    StatusBar.Font.Name = 'Tahoma'
    StatusBar.Font.Style = []
    StatusBar.Color = 10592158
    StatusBar.ColorTo = 5459275
    StatusBar.GradientDirection = gdVertical
    Styler = APS
    FullHeight = 40
    object btnFirst: TSCNavButton
      Left = 105
      Top = 7
      Width = 40
      Height = 25
      Hint = 'First record'
      Caption = 'First'
      DataSource = dsMaster
      ImageIndex = 0
      RoundColor = 13616833
      ShowCaption = False
      Style = scbsCorel
    end
    object btnPrior: TSCNavButton
      Left = 145
      Top = 7
      Width = 40
      Height = 25
      Hint = 'Prior record'
      Caption = 'Prior'
      DataSource = dsMaster
      ImageIndex = 0
      NavType = scntPrior
      RoundColor = 13616833
      ShowCaption = False
      Style = scbsCorel
    end
    object btnNext: TSCNavButton
      Left = 185
      Top = 7
      Width = 40
      Height = 25
      Hint = 'Next record'
      Caption = 'Next'
      DataSource = dsMaster
      ImageIndex = 0
      NavType = scntNext
      RoundColor = 13616833
      ShowCaption = False
      Style = scbsCorel
    end
    object btnLast: TSCNavButton
      Left = 225
      Top = 7
      Width = 40
      Height = 25
      Hint = 'Last record'
      Caption = 'Last'
      DataSource = dsMaster
      ImageIndex = 0
      NavType = scntLast
      RoundColor = 13616833
      ShowCaption = False
      Style = scbsCorel
    end
    object btnSave: TSCButton
      Left = 281
      Top = 7
      Width = 72
      Height = 25
      Caption = 'Simpan'
      Font.Charset = ANSI_CHARSET
      Font.Color = 7485192
      Font.Height = -11
      Font.Name = 'Arial'
      Font.Style = []
      Images = DM.ImageList1
      ImageIndex = 3
      ParentFont = False
      RoundColor = 13616833
      Spacing = 2
      Style = scbsCorel
      TabOrder = 5
      TabStop = False
      OnClick = actSaveExecute
    end
    object btnCancel: TSCButton
      Left = 356
      Top = 7
      Width = 72
      Height = 25
      Caption = 'Batal'
      Font.Charset = ANSI_CHARSET
      Font.Color = 7485192
      Font.Height = -11
      Font.Name = 'Arial'
      Font.Style = []
      Images = DM.ImageList1
      ImageIndex = 2
      ParentFont = False
      RoundColor = 13616833
      Spacing = 2
      Style = scbsCorel
      TabOrder = 6
      TabStop = False
      OnClick = actCancelExecute
    end
    object btnClose: TSCButton
      Left = 15
      Top = 7
      Width = 72
      Height = 25
      Caption = 'Tutup'
      Font.Charset = ANSI_CHARSET
      Font.Color = 7485192
      Font.Height = -11
      Font.Name = 'Arial'
      Font.Style = []
      Images = DM.ImageList1
      ImageIndex = 6
      ParentFont = False
      RoundColor = 13616833
      Spacing = 2
      Style = scbsCorel
      TabOrder = 0
      TabStop = False
      OnClick = actCloseExecute
    end
    object btnAdd: TSCButton
      Left = 281
      Top = 7
      Width = 72
      Height = 25
      Caption = 'Tambah'
      Font.Charset = ANSI_CHARSET
      Font.Color = 7485192
      Font.Height = -11
      Font.Name = 'Arial'
      Font.Style = []
      Images = DM.ImageList1
      ImageIndex = 1
      ParentFont = False
      RoundColor = 13616833
      Spacing = 2
      Style = scbsCorel
      TabOrder = 1
      TabStop = False
      OnClick = actAddExecute
    end
    object btnEdit: TSCButton
      Left = 356
      Top = 7
      Width = 72
      Height = 25
      Caption = 'Koreksi'
      Font.Charset = ANSI_CHARSET
      Font.Color = 7485192
      Font.Height = -11
      Font.Name = 'Arial'
      Font.Style = []
      Images = DM.ImageList1
      ImageIndex = 0
      ParentFont = False
      RoundColor = 13616833
      Spacing = 2
      Style = scbsCorel
      TabOrder = 2
      TabStop = False
      OnClick = actEditExecute
    end
    object btnDelete: TSCButton
      Left = 431
      Top = 7
      Width = 72
      Height = 25
      Caption = 'Hapus'
      Font.Charset = ANSI_CHARSET
      Font.Color = 7485192
      Font.Height = -11
      Font.Name = 'Arial'
      Font.Style = []
      Images = DM.ImageList1
      ImageIndex = 4
      ParentFont = False
      RoundColor = 13616833
      Spacing = 2
      Style = scbsCorel
      TabOrder = 3
      TabStop = False
      OnClick = actDeleteExecute
    end
    object btnRefresh: TSCButton
      Left = 506
      Top = 7
      Width = 72
      Height = 25
      Caption = 'Refresh'
      Font.Charset = ANSI_CHARSET
      Font.Color = 7485192
      Font.Height = -11
      Font.Name = 'Arial'
      Font.Style = []
      Images = DM.ImageList1
      ImageIndex = 5
      ParentFont = False
      RoundColor = 13616833
      Spacing = 2
      Style = scbsCorel
      TabOrder = 4
      TabStop = False
      OnClick = actRefreshExecute
    end
    object btnSearch: TSCButton
      Left = 599
      Top = 7
      Width = 72
      Height = 25
      Caption = 'Cari'
      Font.Charset = ANSI_CHARSET
      Font.Color = 7485192
      Font.Height = -11
      Font.Name = 'Arial'
      Font.Style = []
      Images = DM.ImageList1
      ImageIndex = 7
      ParentFont = False
      RoundColor = 13616833
      Spacing = 2
      Style = scbsCorel
      TabOrder = 7
      TabStop = False
      OnClick = actSearchExecute
    end
    object btnData: TSCButton
      Left = 795
      Top = 8
      Width = 120
      Height = 25
      Caption = 'Pemeliharaan Data'
      Font.Charset = ANSI_CHARSET
      Font.Color = 7485192
      Font.Height = -11
      Font.Name = 'Arial'
      Font.Style = []
      Images = DM.ImageList1
      ImageIndex = 12
      ParentFont = False
      RoundColor = 13616833
      Spacing = 2
      Style = scbsCorel
      TabOrder = 8
      TabStop = False
      Visible = False
      OnClick = btnDataClick
    end
    object btnExport: TSCButton
      Left = 676
      Top = 7
      Width = 113
      Height = 25
      Caption = 'Export To Excel'
      Images = DM.ImageList1
      ImageIndex = 23
      RoundColor = 11507049
      RoundWithParentColor = False
      Style = scbsCorel
      TabOrder = 9
      OnClick = btnExportClick
    end
  end
  object pgMaster: TcxPageControl
    Left = 0
    Top = 81
    Width = 908
    Height = 442
    ActivePage = tsBrowse
    Align = alClient
    Style = 8
    TabOrder = 3
    OnPageChanging = pgMasterPageChanging
    ClientRectBottom = 442
    ClientRectRight = 908
    ClientRectTop = 24
    object tsBrowse: TcxTabSheet
      Caption = 'Browse'
      ImageIndex = 0
      object grdMaster: TcxGrid
        Left = 0
        Top = 0
        Width = 908
        Height = 379
        Align = alClient
        TabOrder = 0
        object grddbtvMaster: TcxGridDBTableView
          OnDblClick = grddbtvMasterDblClick
          NavigatorButtons.ConfirmDelete = False
          DataController.DataSource = dsMaster
          DataController.Summary.DefaultGroupSummaryItems = <>
          DataController.Summary.FooterSummaryItems = <>
          DataController.Summary.SummaryGroups = <>
          OptionsData.Deleting = False
          OptionsData.DeletingConfirmation = False
          OptionsData.Editing = False
          OptionsData.Inserting = False
          OptionsSelection.CellSelect = False
          OptionsView.Indicator = True
          Styles.ContentEven = cxStyle3
          Styles.Selection = cxStyle2
          Styles.Header = cxStyle1
          Styles.Indicator = cxStyle4
          object grddbtvMasterid_rekanan: TcxGridDBColumn
            Caption = 'Kode'
            DataBinding.FieldName = 'kd_rekanan'
            HeaderAlignmentHorz = taCenter
            HeaderAlignmentVert = vaCenter
            HeaderGlyphAlignmentHorz = taCenter
            Options.Editing = False
            Width = 71
          end
          object grddbtvMasternama_rekanan: TcxGridDBColumn
            Caption = 'Nama Rekanan'
            DataBinding.FieldName = 'nama_rekanan'
            HeaderAlignmentHorz = taCenter
            HeaderAlignmentVert = vaCenter
            HeaderGlyphAlignmentHorz = taCenter
            Options.Editing = False
            Width = 146
          end
          object grddbtvMasteralamat: TcxGridDBColumn
            Caption = 'Alamat'
            DataBinding.FieldName = 'alamat'
            HeaderAlignmentHorz = taCenter
            HeaderAlignmentVert = vaCenter
            HeaderGlyphAlignmentHorz = taCenter
            Options.Editing = False
            Width = 184
          end
          object grddbtvMasterkd_gol: TcxGridDBColumn
            Caption = 'Gol. Tarif'
            DataBinding.FieldName = 'kd_gol'
            HeaderAlignmentHorz = taCenter
            HeaderAlignmentVert = vaCenter
            Options.Editing = False
          end
          object grddbtvMasterkota: TcxGridDBColumn
            Caption = 'Kota'
            DataBinding.FieldName = 'kota'
            HeaderAlignmentHorz = taCenter
            HeaderAlignmentVert = vaCenter
            HeaderGlyphAlignmentHorz = taCenter
            Options.Editing = False
            Width = 100
          end
          object grddbtvMastertelepon: TcxGridDBColumn
            Caption = 'Telepon'
            DataBinding.FieldName = 'telephone'
            HeaderAlignmentHorz = taCenter
            HeaderAlignmentVert = vaCenter
            HeaderGlyphAlignmentHorz = taCenter
            Options.Editing = False
            Width = 82
          end
          object grddbtvMasterfax: TcxGridDBColumn
            Caption = 'Faxcimile'
            DataBinding.FieldName = 'faxcimile'
            HeaderAlignmentHorz = taCenter
            HeaderAlignmentVert = vaCenter
            HeaderGlyphAlignmentHorz = taCenter
            Options.Editing = False
            Width = 66
          end
          object grddbtvMastercontact: TcxGridDBColumn
            Caption = 'Kontak'
            DataBinding.FieldName = 'contact'
            HeaderAlignmentHorz = taCenter
            HeaderAlignmentVert = vaCenter
            HeaderGlyphAlignmentHorz = taCenter
            Options.Editing = False
            Width = 100
          end
          object grddbtvMasterhp: TcxGridDBColumn
            Caption = 'Hand Phone'
            DataBinding.FieldName = 'mobile_no'
            HeaderAlignmentHorz = taCenter
            HeaderAlignmentVert = vaCenter
            HeaderGlyphAlignmentHorz = taCenter
            Options.Editing = False
            Width = 100
          end
        end
        object grdMasterDBTableView2: TcxGridDBTableView
          NavigatorButtons.ConfirmDelete = False
          DataController.Summary.DefaultGroupSummaryItems = <>
          DataController.Summary.FooterSummaryItems = <>
          DataController.Summary.SummaryGroups = <>
          object grdMasterDBTableView2Column1: TcxGridDBColumn
          end
        end
        object grdlvlMaster: TcxGridLevel
          GridView = grddbtvMaster
        end
      end
      object pnlSearch: TSCPanel
        Left = 0
        Top = 379
        Width = 908
        Height = 39
        Align = alBottom
        Color = 15986925
        GradientMid = 15845560
        GradientUsesMid = True
        ParentColor = False
        TabOrder = 1
        Visible = False
        object Shape1: TShape
          Left = 11
          Top = 5
          Width = 97
          Height = 29
        end
        object Label1: TLabel
          Left = 19
          Top = 13
          Width = 69
          Height = 13
          Caption = 'Nama Kolom : '
          Transparent = True
        end
        object Shape2: TShape
          Left = 107
          Top = 5
          Width = 507
          Height = 29
        end
        object edtSearch: TwwIncrementalSearch
          Left = 240
          Top = 10
          Width = 364
          Height = 19
          DataSource = dsMaster
          ShowMatchText = True
          Ctl3D = False
          ParentCtl3D = False
          TabOrder = 0
        end
        object cbFieldName: TcxComboBox
          Left = 113
          Top = 9
          Properties.OnChange = cbFieldNamePropertiesChange
          TabOrder = 1
          Width = 121
        end
      end
    end
    object tsDetail: TcxTabSheet
      Caption = 'Detail'
      ImageIndex = 1
      object pnlDetailSupplier: TRzPanel
        Left = 13
        Top = 6
        Width = 458
        Height = 555
        Color = 15919073
        Ctl3D = True
        ParentCtl3D = False
        TabOrder = 0
        object lblDBNamaRekanan: TRzDBLabel
          Left = 12
          Top = 8
          Width = 301
          Height = 22
          Font.Charset = DEFAULT_CHARSET
          Font.Color = 7879740
          Font.Height = -19
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          ParentFont = False
          Transparent = True
          BlinkIntervalOff = 600
          BlinkIntervalOn = 600
          DataField = 'nama_rekanan'
          DataSource = dsMaster
        end
        object RzLabel1: TRzLabel
          Left = 13
          Top = 33
          Width = 69
          Height = 13
          Caption = 'Kode Rekanan'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
          BlinkIntervalOff = 600
          BlinkIntervalOn = 600
          BevelWidth = 0
        end
        object RzLabel2: TRzLabel
          Left = 13
          Top = 79
          Width = 72
          Height = 13
          Caption = 'Nama Rekanan'
          FocusControl = EdtFirstName
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
          BlinkIntervalOff = 600
          BlinkIntervalOn = 600
          BevelWidth = 0
        end
        object RzLabel3: TRzLabel
          Left = 227
          Top = 78
          Width = 63
          Height = 13
          Caption = 'Nama Kontak'
          FocusControl = EdtContact
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
          BlinkIntervalOff = 600
          BlinkIntervalOn = 600
          BevelWidth = 0
        end
        object RzLabel4: TRzLabel
          Left = 13
          Top = 127
          Width = 33
          Height = 13
          Caption = 'Alamat'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
          BlinkIntervalOff = 600
          BlinkIntervalOn = 600
          BevelWidth = 0
        end
        object RzLabel5: TRzLabel
          Left = 13
          Top = 170
          Width = 22
          Height = 13
          Caption = 'Kota'
          FocusControl = EdtCity
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
          BlinkIntervalOff = 600
          BlinkIntervalOn = 600
          BevelWidth = 0
        end
        object RzLabel9: TRzLabel
          Left = 13
          Top = 216
          Width = 38
          Height = 13
          Caption = 'Telepon'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
          BlinkIntervalOff = 600
          BlinkIntervalOn = 600
          BevelWidth = 0
        end
        object RzLabel10: TRzLabel
          Left = 227
          Top = 215
          Width = 18
          Height = 13
          Caption = 'Fax'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
          BlinkIntervalOff = 600
          BlinkIntervalOn = 600
          BevelWidth = 0
        end
        object Label2: TLabel
          Left = 63
          Top = 46
          Width = 12
          Height = 19
          AutoSize = False
          Caption = '.'
          Font.Charset = ANSI_CHARSET
          Font.Color = clWindowText
          Font.Height = -16
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object RzLabel6: TRzLabel
          Left = 13
          Top = 324
          Width = 69
          Height = 13
          Caption = 'Plafon Piutang'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
          BlinkIntervalOff = 600
          BlinkIntervalOn = 600
          BevelWidth = 0
        end
        object RzLabel7: TRzLabel
          Left = 229
          Top = 269
          Width = 31
          Height = 13
          Caption = 'NPPKP'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
          BlinkIntervalOff = 600
          BlinkIntervalOn = 600
          BevelWidth = 0
        end
        object RzLabel8: TRzLabel
          Left = 13
          Top = 270
          Width = 29
          Height = 13
          Caption = 'NPWP'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
          BlinkIntervalOff = 600
          BlinkIntervalOn = 600
          BevelWidth = 0
        end
        object Shape6: TShape
          Left = 226
          Top = 186
          Width = 212
          Height = 19
        end
        object RzLabel12: TRzLabel
          Left = 228
          Top = 172
          Width = 68
          Height = 13
          Caption = 'PKP/ NON PKP'
          FocusControl = EdtCity
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
          BlinkIntervalOff = 600
          BlinkIntervalOn = 600
          BevelWidth = 0
        end
        object RzLabel11: TRzLabel
          Left = 14
          Top = 377
          Width = 94
          Height = 13
          Caption = 'Diskon Member (%)'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
          BlinkIntervalOff = 600
          BlinkIntervalOn = 600
          BevelWidth = 0
        end
        object RzLabel13: TRzLabel
          Left = 158
          Top = 377
          Width = 109
          Height = 13
          Caption = 'Range Total Pembelian'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
          BlinkIntervalOff = 600
          BlinkIntervalOn = 600
          BevelWidth = 0
        end
        object RzLabel14: TRzLabel
          Left = 263
          Top = 396
          Width = 25
          Height = 13
          Caption = 'S./D.'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
          BlinkIntervalOff = 600
          BlinkIntervalOn = 600
          BevelWidth = 0
        end
        object EdtFirstName: TRzDBEdit
          Left = 12
          Top = 94
          Width = 215
          Height = 19
          DataSource = dsMaster
          DataField = 'nama_rekanan'
          Ctl3D = False
          Font.Charset = ANSI_CHARSET
          Font.Color = 7879740
          Font.Height = -11
          Font.Name = 'Verdana'
          Font.Style = [fsBold]
          ParentCtl3D = False
          ParentFont = False
          TabOrder = 1
          OnKeyPress = EditKeyPress
        end
        object EdtAddress: TRzDBEdit
          Left = 12
          Top = 141
          Width = 428
          Height = 19
          DataSource = dsMaster
          DataField = 'alamat'
          Ctl3D = False
          Font.Charset = ANSI_CHARSET
          Font.Color = 7879740
          Font.Height = -11
          Font.Name = 'Verdana'
          Font.Style = [fsBold]
          ParentCtl3D = False
          ParentFont = False
          TabOrder = 3
          OnKeyPress = EditKeyPress
        end
        object EdtCity: TRzDBEdit
          Left = 12
          Top = 186
          Width = 215
          Height = 19
          DataSource = dsMaster
          DataField = 'kota'
          Ctl3D = False
          Font.Charset = ANSI_CHARSET
          Font.Color = 7879740
          Font.Height = -11
          Font.Name = 'Verdana'
          Font.Style = [fsBold]
          ParentCtl3D = False
          ParentFont = False
          TabOrder = 4
          OnKeyPress = EditKeyPress
        end
        object EdtPhone: TRzDBEdit
          Left = 12
          Top = 231
          Width = 215
          Height = 19
          DataSource = dsMaster
          DataField = 'telephone'
          Ctl3D = False
          Font.Charset = ANSI_CHARSET
          Font.Color = 7879740
          Font.Height = -11
          Font.Name = 'Verdana'
          Font.Style = [fsBold]
          ParentCtl3D = False
          ParentFont = False
          TabOrder = 6
          OnKeyPress = EditKeyPress
        end
        object EdtFax: TRzDBEdit
          Left = 226
          Top = 231
          Width = 215
          Height = 19
          DataSource = dsMaster
          DataField = 'faxcimile'
          Ctl3D = False
          Font.Charset = ANSI_CHARSET
          Font.Color = 7879740
          Font.Height = -11
          Font.Name = 'Verdana'
          Font.Style = [fsBold]
          ParentCtl3D = False
          ParentFont = False
          TabOrder = 7
          OnKeyPress = EditKeyPress
        end
        object edtid_Supplier: TRzDBEdit
          Left = 12
          Top = 49
          Width = 215
          Height = 19
          DataSource = dsMaster
          DataField = 'kd_rekanan'
          Ctl3D = False
          Font.Charset = ANSI_CHARSET
          Font.Color = 7879740
          Font.Height = -11
          Font.Name = 'Verdana'
          Font.Style = [fsBold]
          ParentCtl3D = False
          ParentFont = False
          TabOrder = 0
          OnKeyPress = EditKeyPress
        end
        object EdtContact: TRzDBEdit
          Left = 226
          Top = 94
          Width = 215
          Height = 19
          DataSource = dsMaster
          DataField = 'contact'
          Ctl3D = False
          Font.Charset = ANSI_CHARSET
          Font.Color = 7879740
          Font.Height = -11
          Font.Name = 'Verdana'
          Font.Style = [fsBold]
          ParentCtl3D = False
          ParentFont = False
          TabOrder = 2
          OnKeyPress = EditKeyPress
        end
        object edtPlafon: TRzDBEdit
          Left = 12
          Top = 339
          Width = 215
          Height = 19
          DataSource = dsMaster
          DataField = 'plafon'
          Ctl3D = False
          Font.Charset = ANSI_CHARSET
          Font.Color = 7879740
          Font.Height = -11
          Font.Name = 'Verdana'
          Font.Style = [fsBold]
          ParentCtl3D = False
          ParentFont = False
          TabOrder = 10
          OnKeyPress = EditKeyPress
        end
        object edtNPWP: TRzDBEdit
          Left = 12
          Top = 285
          Width = 215
          Height = 19
          DataSource = dsMaster
          DataField = 'npwp'
          Ctl3D = False
          Font.Charset = ANSI_CHARSET
          Font.Color = 7879740
          Font.Height = -11
          Font.Name = 'Verdana'
          Font.Style = [fsBold]
          ParentCtl3D = False
          ParentFont = False
          TabOrder = 8
          OnKeyPress = EditKeyPress
        end
        object edtNPPKP: TRzDBEdit
          Left = 226
          Top = 285
          Width = 215
          Height = 19
          DataSource = dsMaster
          DataField = 'nppkp'
          Ctl3D = False
          Font.Charset = ANSI_CHARSET
          Font.Color = 7879740
          Font.Height = -11
          Font.Name = 'Verdana'
          Font.Style = [fsBold]
          ParentCtl3D = False
          ParentFont = False
          TabOrder = 9
          OnKeyPress = EditKeyPress
        end
        object ckbDetail: TcxDBCheckBox
          Left = 232
          Top = 186
          Caption = 'PKP'
          DataBinding.DataField = 'is_pkp'
          DataBinding.DataSource = dsMaster
          Properties.NullStyle = nssUnchecked
          Properties.ValueChecked = '1'
          Properties.ValueUnchecked = '0'
          TabOrder = 5
          Transparent = True
          OnKeyPress = EditKeyPress
          Width = 210
        end
        object edtDisc: TcxDBTextEdit
          Left = 11
          Top = 392
          DataBinding.DataField = 'disc_member_psn'
          DataBinding.DataSource = dsMaster
          Style.Shadow = False
          StyleFocused.BorderStyle = ebsThick
          StyleHot.BorderStyle = ebsThick
          TabOrder = 11
          OnKeyPress = EditKeyPress
          Width = 134
        end
        object edtTotalMin: TcxDBTextEdit
          Left = 155
          Top = 392
          DataBinding.DataField = 'total_min'
          DataBinding.DataSource = dsMaster
          Style.Shadow = False
          StyleFocused.BorderStyle = ebsThick
          StyleHot.BorderStyle = ebsThick
          TabOrder = 12
          OnKeyPress = EditKeyPress
          Width = 102
        end
        object edtTotalMax: TcxDBTextEdit
          Left = 294
          Top = 392
          DataBinding.DataField = 'total_max'
          DataBinding.DataSource = dsMaster
          Style.Shadow = False
          StyleFocused.BorderStyle = ebsThick
          StyleHot.BorderStyle = ebsThick
          TabOrder = 13
          OnKeyPress = EditKeyPress
          Width = 102
        end
      end
    end
  end
  object AFS: TAdvFormStyler
    Style = tsOffice2007Obsidian
    AppStyle = DM.apsMain
    Left = 158
    Top = 22
  end
  object APS: TAdvPanelStyler
    Tag = 0
    Settings.AnchorHint = False
    Settings.AutoHideChildren = False
    Settings.BevelInner = bvNone
    Settings.BevelOuter = bvNone
    Settings.BevelWidth = 1
    Settings.BorderColor = 16765615
    Settings.BorderShadow = False
    Settings.BorderStyle = bsNone
    Settings.BorderWidth = 0
    Settings.CanMove = False
    Settings.CanSize = False
    Settings.Caption.Color = 12105910
    Settings.Caption.ColorTo = 10526878
    Settings.Caption.Font.Charset = DEFAULT_CHARSET
    Settings.Caption.Font.Color = clWhite
    Settings.Caption.Font.Height = -11
    Settings.Caption.Font.Name = 'MS Sans Serif'
    Settings.Caption.Font.Style = []
    Settings.Caption.GradientDirection = gdVertical
    Settings.Caption.Indent = 2
    Settings.Caption.ShadeLight = 255
    Settings.Collaps = False
    Settings.CollapsColor = clNone
    Settings.CollapsDelay = 0
    Settings.CollapsSteps = 0
    Settings.Color = 13616833
    Settings.ColorTo = 12958644
    Settings.ColorMirror = 12958644
    Settings.ColorMirrorTo = 15527141
    Settings.Cursor = crDefault
    Settings.Font.Charset = DEFAULT_CHARSET
    Settings.Font.Color = 7485192
    Settings.Font.Height = -11
    Settings.Font.Name = 'MS Sans Serif'
    Settings.Font.Style = []
    Settings.FixedTop = False
    Settings.FixedLeft = False
    Settings.FixedHeight = False
    Settings.FixedWidth = False
    Settings.Height = 120
    Settings.Hover = False
    Settings.HoverColor = clNone
    Settings.HoverFontColor = clNone
    Settings.Indent = 0
    Settings.ShadowColor = clBlack
    Settings.ShadowOffset = 0
    Settings.ShowHint = False
    Settings.ShowMoveCursor = False
    Settings.StatusBar.BorderStyle = bsSingle
    Settings.StatusBar.Font.Charset = DEFAULT_CHARSET
    Settings.StatusBar.Font.Color = clWhite
    Settings.StatusBar.Font.Height = -11
    Settings.StatusBar.Font.Name = 'Tahoma'
    Settings.StatusBar.Font.Style = []
    Settings.StatusBar.Color = 10592158
    Settings.StatusBar.ColorTo = 5459275
    Settings.StatusBar.GradientDirection = gdVertical
    Settings.TextVAlign = tvaTop
    Settings.TopIndent = 0
    Settings.URLColor = clBlue
    Settings.Width = 0
    Style = psOffice2007Obsidian
    Left = 348
    Top = 20
  end
  object SR: TcxStyleRepository
    Left = 192
    Top = 26
    PixelsPerInch = 96
    object cxStyle1: TcxStyle
      AssignedValues = [svColor, svFont, svTextColor]
      Color = 7039851
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      TextColor = clWhite
    end
    object cxStyle2: TcxStyle
      AssignedValues = [svColor, svFont, svTextColor]
      Color = 14680063
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      TextColor = clBlack
    end
    object cxStyle3: TcxStyle
      AssignedValues = [svColor, svFont, svTextColor]
      Color = 15400938
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      TextColor = clBlack
    end
    object cxStyle4: TcxStyle
      AssignedValues = [svColor, svFont, svTextColor]
      Color = 15587527
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      TextColor = clBlack
    end
    object cxGridTableViewStyleSheet1: TcxGridTableViewStyleSheet
      Styles.ContentEven = cxStyle3
      Styles.Selection = cxStyle2
      Styles.Header = cxStyle1
      Styles.Indicator = cxStyle4
      BuiltIn = True
    end
    object cxGridBandedTableViewStyleSheet1: TcxGridBandedTableViewStyleSheet
      Styles.ContentEven = cxStyle3
      Styles.Selection = cxStyle2
      Styles.Indicator = cxStyle4
      Styles.BandHeader = cxStyle1
      BuiltIn = True
    end
  end
  object dsMaster: TDataSource
    DataSet = Master
    Left = 132
    Top = 42
  end
  object Master: TZQuery
    Connection = DM.conn
    BeforePost = MasterBeforePost
    OnNewRecord = MasterNewRecord
    SQL.Strings = (
      'select a.* from master.mrekanan a'
      'where a.jenis='#39'CUSTOMER'#39)
    Params = <>
    Left = 121
    Top = 21
    object Masterkd_rekanan: TStringField
      FieldName = 'kd_rekanan'
    end
    object Masternama_rekanan: TStringField
      FieldName = 'nama_rekanan'
      Size = 255
    end
    object Masteralamat: TStringField
      FieldName = 'alamat'
      Size = 255
    end
    object Masterjenis: TStringField
      FieldName = 'jenis'
      Size = 50
    end
    object Mastercontact: TStringField
      FieldName = 'contact'
      Size = 50
    end
    object Masterkota: TStringField
      FieldName = 'kota'
      Size = 30
    end
    object Masterkode_pos: TStringField
      FieldName = 'kode_pos'
      Size = 10
    end
    object Mastertelephone: TStringField
      FieldName = 'telephone'
      Size = 100
    end
    object Masterfaxcimile: TStringField
      FieldName = 'faxcimile'
      Size = 100
    end
    object Masteralamat2: TStringField
      FieldName = 'alamat2'
      Size = 255
    end
    object Masterprovince: TStringField
      FieldName = 'province'
      Size = 100
    end
    object Mastercountry: TStringField
      FieldName = 'country'
      Size = 100
    end
    object Masteremail: TStringField
      FieldName = 'email'
      Size = 100
    end
    object Masterremarks: TStringField
      FieldName = 'remarks'
      Size = 255
    end
    object Masterusr_ins: TStringField
      FieldName = 'usr_ins'
      Size = 30
    end
    object Masterusr_upd: TStringField
      FieldName = 'usr_upd'
      Size = 30
    end
    object Mastercontact_title: TStringField
      FieldName = 'contact_title'
      Size = 50
    end
    object Mastermobile_no: TStringField
      FieldName = 'mobile_no'
      Size = 100
    end
    object Masterplafon: TFloatField
      FieldName = 'plafon'
    end
    object Masternpwp: TStringField
      FieldName = 'npwp'
      Size = 50
    end
    object Masternppkp: TStringField
      FieldName = 'nppkp'
      Size = 50
    end
    object Masteris_pkp: TStringField
      FieldName = 'is_pkp'
      Size = 1
    end
    object Masterdisc_member_psn: TFloatField
      FieldName = 'disc_member_psn'
      Required = True
    end
    object Mastertotal_min: TFloatField
      FieldName = 'total_min'
      Required = True
      DisplayFormat = ',0'
    end
    object Mastertotal_max: TFloatField
      FieldName = 'total_max'
      Required = True
      DisplayFormat = ',0'
    end
  end
  object aclFrm: TActionList
    Left = 234
    Top = 26
    object actClose: TAction
      Caption = 'Tutup'
      OnExecute = actCloseExecute
    end
    object actAdd: TAction
      Caption = 'Tambah'
      OnExecute = actAddExecute
    end
    object actEdit: TAction
      Caption = 'Koreksi'
      OnExecute = actEditExecute
    end
    object actDelete: TAction
      Caption = 'Hapus'
      OnExecute = actDeleteExecute
    end
    object actRefresh: TAction
      Caption = 'Refresh'
      OnExecute = actRefreshExecute
    end
    object actSave: TAction
      Caption = 'Simpan'
      OnExecute = actSaveExecute
    end
    object actCancel: TAction
      Caption = 'Batal'
      OnExecute = actCancelExecute
    end
    object actSearch: TAction
      Caption = 'actSearch'
      OnExecute = actSearchExecute
    end
  end
  object qryGen_ID_Supplier: TZReadOnlyQuery
    Connection = DM.conn
    SQL.Strings = (
      'select master.fn_gen_id_supplier(:pjenis)')
    Params = <
      item
        DataType = ftUnknown
        Name = 'pjenis'
        ParamType = ptUnknown
      end>
    Left = 462
    Top = 22
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'pjenis'
        ParamType = ptUnknown
      end>
    object qryGen_ID_Supplierfn_gen_id_supplier: TStringField
      FieldName = 'fn_gen_id_supplier'
      ReadOnly = True
      Size = 255
    end
  end
  object OpenDialog: TSaveDialog
    DefaultExt = 'xls'
    Filter = '.xls'
    Left = 419
    Top = 12
  end
end
