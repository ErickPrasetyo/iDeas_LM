...
uses
  ...
  dxSkinsDefaultPainters, dxSkinscxPCPainter, cxDropDownEdit, dxSkinBlack,
...
type
  ...
  TUserMenuFrm = class(TForm)
    ...
    Detailis_preparer: TStringField;
    Detailis_reviewer: TStringField;
    Detailis_approver: TStringField;
    grddbtvDetailis_reviewer: TcxGridDBColumn;
    grddbtvDetailis_approver: TcxGridDBColumn;
    Label4: TLabel;
    Masterusr_principal: TStringField;
    edtPrincipal: TcxDBComboBox;
...
//  if Masterusr_principal.IsNull or (Trim(Masterusr_principal.AsString)='') then
//    raise Exception.Create('ADMIN OPERATOR harus diisi !');

  if Masterpasswd.IsNull or (Trim(Masterpasswd.AsString)='')then
    Masterpasswd.AsString:= MD5Print(MD5String(''));

  if edtPrincipal.Text= 'MAYORA1' then
     Masterusr_principal.AsString:= 'MYR1';
  if edtPrincipal.Text= 'MAYORA2' then
     Masterusr_principal.AsString:= 'MYR2';
  if edtPrincipal.Text= 'MAYORA3' then
     Masterusr_principal.AsString:= 'MYR3';
  if edtPrincipal.Text= 'MAYORA4' then
     Masterusr_principal.AsString:= 'MYR4';
  if edtPrincipal.Text= 'MAYORA5' then
     Masterusr_principal.AsString:= 'MYR5';
  if edtPrincipal.Text= 'BENDERA' then
     Masterusr_principal.AsString:= 'BDR';
  if edtPrincipal.Text= 'GARUDA' then
     Masterusr_principal.AsString:= 'GRD';
...
begin
  ...
  else if SameText(s,'Export') then
    old_val:= Detailisexport.Value;
...
procedure TUserMenuFrm.DetailNewRecord(DataSet: TDataSet);
begin
  Detailis_preparer.AsString:= '0';
  Detailis_reviewer.AsString:= '0';
  Detailis_approver.AsString:= '0';
end;
