...
var
  ImportHutang2Frm: TImportHutang2Frm;
...
function TImportHutang2Frm.GetMonthNum(vMonth:String):integer;
begin
   Result:= 0;
...
function TImportHutang2Frm.CekString(vString:string):Boolean ;
begin
   Result:= True;
...
procedure TImportHutang2Frm.setButton(state:string);
begin
...
procedure TImportHutang2Frm.ConnectToExcel;
...
procedure TImportHutang2Frm.FetchData;
begin
...
procedure TImportHutang2Frm.edtFileExcelPropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
begin
...
procedure TImportHutang2Frm.FormCreate(Sender: TObject);
begin
...
procedure TImportHutang2Frm.btnFetchClick(Sender: TObject);
begin
...
procedure TImportHutang2Frm.ImportMaster;
var
    dtF, dtT : TDateTime;
    iq, i, cnt: Integer;
    isNewNota : boolean;
    s, opr, F1str, vNoNota : string;
              ...
              for iq:= 0  to kmtMemorial.Fields.Count-1  do begin
                  if iq=0 then begin
                      if UpperCase(copy(kmtMemorial.Fields[iq].AsString,1,9))=UpperCase('No.Faktur') then begin
                           ...
                           dtF:= EncodeDate(StrToInt(copy(F1str,36,4)),
                                                     GetMonthNum(copy(F1str,33,3)),
                                                      StrToInt(copy(F1str,31,2)));

                           dtT:= EncodeDate(StrToInt(copy(F1str,55,4)),
                                                     GetMonthNum(copy(F1str,52,3)),
                                                     StrToInt(copy(F1str,50,2)));
                      end
...
procedure TImportHutang2Frm.ImportDetail;
var
    iq, i, cnt: Integer;
    isPCCode : boolean;
...
procedure TImportHutang2Frm.ImportError;
var s : String;
begin
...
procedure TImportHutang2Frm.GoToTheRealTable;
var s : String;
begin
  try
...
procedure TImportHutang2Frm.FormShow(Sender: TObject);
begin
  LblStatus.Caption:= '';
  ...
  LblSupplier.Caption:= 'BUNGAH';
  setButton('F');
  edtSupplier.Properties.ReadOnly:= True;
  edtSupplier.Properties.Buttons[0].Visible:= False;
end;

procedure TImportHutang2Frm.tabImportErrorShow(Sender: TObject);
begin
   try
     QTableError.Close;
     QTableError.Open;
...
procedure TImportHutang2Frm.btnImportClick(Sender: TObject);
begin
...
procedure TImportHutang2Frm.edtSupplierPropertiesButtonClick(
...
procedure TImportHutang2Frm.tabImportDataShow(Sender: TObject);
begin
  lblHeader.Caption:= 'Import Data Faktur Pembelian';
end;

procedure TImportHutang2Frm.btnInsertErrorClick(Sender: TObject);
begin
...
function TImportHutang2Frm.FixedErrorData:Boolean;
var s : String;
   i, cnt: Integer;
begin
     Result:= False;
