...
uses
  ...
  cxGridDBBandedTableView, DBCtrls, ExtCtrls, cxDropDownEdit, cxLookupEdit,
  cxDBLookupEdit, cxDBLookupComboBox, cxMemo, cxDBLabel, cxGroupBox,
  RzPanel, cxMaskEdit, cxCalendar, cxContainer, StdCtrls,
...
begin
    try
      KasirTunaiFrm := TKasirTunaiFrm.Create(Application);
      with KasirTunaiFrm do begin
...
procedure TKasirTunaiFrm.CloneDetail;
begin
...
function TKasirTunaiFrm.CheckDetail(id_item: string): Boolean;
begin
  Result:= False;
  try
...
function TKasirTunaiFrm.CheckEditRight(var msg: string): Boolean;
begin
  Result:= True;
  msg:= '';
...
function TKasirTunaiFrm.CheckDeleteRight(var msg: string): Boolean;
begin
  Result:= True;
  msg:= '';
...
procedure TKasirTunaiFrm.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action:= caFree;
end;

procedure TKasirTunaiFrm.FormCloseQuery(Sender: TObject;
...
procedure TKasirTunaiFrm.actCloseExecute(Sender: TObject);
begin
  Close;
end;

procedure TKasirTunaiFrm.actAddExecute(Sender: TObject);
begin
...
procedure TKasirTunaiFrm.actEditExecute(Sender: TObject);
begin
  if pgTransaction.ActivePageIndex=0 then
     pgTransaction.ActivePageIndex:= 1;
...
procedure TKasirTunaiFrm.actDeleteExecute(Sender: TObject);
begin
  if pgTransaction.ActivePageIndex=0 then
     pgTransaction.ActivePageIndex:= 1;
...
procedure TKasirTunaiFrm.actRefreshExecute(Sender: TObject);
begin
  DBMode:= dmNone;
  try
    qBrowse.Close;
...
procedure TKasirTunaiFrm.actSaveExecute(Sender: TObject);
begin
...
procedure TKasirTunaiFrm.actCancelExecute(Sender: TObject);
begin
  try
    Detail.Cancel;
    Master.Cancel;
...
procedure TKasirTunaiFrm.UpdateView;
...
procedure TKasirTunaiFrm.pgTransactionPageChanging(Sender: TObject;
  NewPage: TcxTabSheet; var AllowChange: Boolean);
begin
  AllowChange:= (DBMode=dmNone) or (DBMode=dmBrowse)
end;

procedure TKasirTunaiFrm.grdDetailEnter(Sender: TObject);
begin
...
//  try
//    L_Nota.Close;
//    L_Nota.Params.ParamByName('pid_supplier').Value:= Masterid_rekanan.AsString;
//    L_Nota.Params.ParamByName('pid_rek_gl').Value:= Masterid_rek_gl.AsString;
//    L_Nota.Open;
...
procedure TKasirTunaiFrm.MasterNewRecord(DataSet: TDataSet);
begin
   ...
   Masterno_kas_kecil.AsString:= 'KT'+DM.L_Perusahaannomor_distributor.AsString+'-'+FormatDateTime('YYMM',DM.vSysDatedt_server.AsDateTime)+'-00000';

end;

procedure TKasirTunaiFrm.DetailNewRecord(DataSet: TDataSet);
begin
...
procedure TKasirTunaiFrm.MasterBeforePost(DataSet: TDataSet);
begin
...
procedure TKasirTunaiFrm.pgTransactionChange(Sender: TObject);
begin
...
procedure TKasirTunaiFrm.grddbtvMasterCellDblClick(
...
procedure TKasirTunaiFrm.DetailBeforeInsert(DataSet: TDataSet);
begin
  if Master.State<>dsBrowse then
     Master.Post;
end;

procedure TKasirTunaiFrm.MasterBeforeOpen(DataSet: TDataSet);
begin
  Master.Params.ParamByName('id_kas_kecil').Value:= qBrowseid_kas_kecil.AsLargeInt;
end;

procedure TKasirTunaiFrm.DetailBeforeEdit(DataSet: TDataSet);
begin
    if Master.State<>dsBrowse then
      Master.Post;
end;

procedure TKasirTunaiFrm.actPrintExecute(Sender: TObject);
...
procedure TKasirTunaiFrm.DetailBeforePost(DataSet: TDataSet);
begin
...
//  if Detailid_rekanan.IsNull or (Trim(Detailid_rekanan.AsString)='') then
//     Detailid_rekanan.Clear;
...
procedure TKasirTunaiFrm.MasterBeforeDelete(DataSet: TDataSet);
var msg: string;
begin
  if Master.RecordCount=0 then
     Exit;
...
procedure TKasirTunaiFrm.MasterBeforeEdit(DataSet: TDataSet);
var msg: string;
begin
  if Master.RecordCount=0 then
    Abort;
...
procedure TKasirTunaiFrm.DetailAfterOpen(DataSet: TDataSet);
begin
  CloneDetail;
end;

procedure TKasirTunaiFrm.DetailAfterPost(DataSet: TDataSet);
begin
  CloneDetail;
end;

procedure TKasirTunaiFrm.edtRekGLEnter(Sender: TObject);
begin
  try
    L_ID_REK_KAS.Close;
    L_ID_REK_KAS.Open;
...
procedure TKasirTunaiFrm.EditKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure TKasirTunaiFrm.edtRemarksKeyPress(Sender: TObject; var Key: Char);
begin
    if Key=#13 then begin
       Detail.Append;
       grdDetail.SetFocus;
       grdDetailDBBTV.GetColumnByFieldName('diskripsi').FocusWithSelection;
    end;
end;

procedure TKasirTunaiFrm.dtpStartCloseUp(Sender: TObject);
var dt: TDateTime;
    y,m,d: Word;
begin
...
procedure TKasirTunaiFrm.dtpEndCloseUp(Sender: TObject);
var dt: TDateTime;
    y,m,d: Word;
begin
...
procedure TKasirTunaiFrm.qBrowseBeforeOpen(DataSet: TDataSet);
begin
...
procedure TKasirTunaiFrm.btnOKClick(Sender: TObject);
begin
   try
     qBrowse.Close;
     qBrowse.Open;
...
procedure TKasirTunaiFrm.tsDetailShow(Sender: TObject);
begin
  try
    Master.Close;
    Master.Open;
...
procedure TKasirTunaiFrm.grdDetailDBBTVid_rek_glGetPropertiesForEdit(
  Sender: TcxCustomGridTableItem; ARecord: TcxCustomGridRecord;
  var AProperties: TcxCustomEditProperties);
begin
...
procedure TKasirTunaiFrm.grdDetailDBBTVid_cc_projectGetPropertiesForEdit(
  Sender: TcxCustomGridTableItem; ARecord: TcxCustomGridRecord;
  var AProperties: TcxCustomEditProperties);
begin
  if Detail.State in [dsInsert,dsedit] then
...
procedure TKasirTunaiFrm.ER_LCB_REFFPropertiesCloseUp(Sender: TObject);
begin
...
//     Detaildiskripsi.AsString:= 'Pembayaran Invoice/Fee No.'+ L_Notano_doc.AsString
//  else Detaildiskripsi.AsString:= L_Notaremarks.AsString;
...
//  Detailid_cc_project.AsString:= L_Notaid_cc_project.AsString;
//  Detailid_rek_gl.AsString:= L_Notaid_rek_gl.AsString;

end;

procedure TKasirTunaiFrm.grdDetailDBBTVno_reffGetPropertiesForEdit(
  Sender: TcxCustomGridTableItem; ARecord: TcxCustomGridRecord;
  var AProperties: TcxCustomEditProperties);
begin
...
procedure TKasirTunaiFrm.grdDetailDBBTVid_arusGetPropertiesForEdit(
  Sender: TcxCustomGridTableItem; ARecord: TcxCustomGridRecord;
  var AProperties: TcxCustomEditProperties);
begin
  if Detail.State in [dsInsert,dsedit] then begin
...
procedure TKasirTunaiFrm.edtCustNoPropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
begin
...
procedure TKasirTunaiFrm.F4KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
   edtCustNoPropertiesButtonClick(Nil,0);
end;

procedure TKasirTunaiFrm.edtDateEditPropertiesChange(Sender: TObject);
begin
//   if Master.State=dsBrowse then
//      Master.edit;
//      Masterno_kas_kecil.AsString:= 'KT'+DM.L_Perusahaannomor_distributor.AsString+'-'+FormatDateTime('YYMM',edtDateEdit.Date)+'-00000';
end;
