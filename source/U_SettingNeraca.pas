...
type
  ...
  TSettingNeracaFrm = class(TForm)
    ...
    Masterid_rek_neraca: TStringField;
    Masterdiskripsi: TStringField;
    Masternperiode_ini: TFloatField;
    Masternperiode_lalu: TFloatField;
...
var
  SettingNeracaFrm: TSettingNeracaFrm;

implementation

uses U_DM, U_LookRekening;

{$R *.dfm}



procedure TSettingNeracaFrm.UpdateView;
...
procedure TSettingNeracaFrm.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action:= caFree
end;

procedure TSettingNeracaFrm.FormShow(Sender: TObject);
begin
  DBMode:= dmNone;
  pgcMaster.ActivePageIndex:= 0;
...
procedure TSettingNeracaFrm.pgcMasterChange(Sender: TObject);
begin
  if (Master.State<>dsInactive) and (Master.RecordCount>0) and (pgcMaster.ActivePageIndex=1) then begin
    try
      Detail.Close;
      Detail.Params.ParamByName('id_rek_neraca').Value:= Masterid_rek_neraca.AsString;
      Detail.Open;
    except
      on E: Exception do begin
        DM.MyMsg(mmError, 'Error has been encountered !',E.Message)
...
procedure TSettingNeracaFrm.pgcMasterPageChanging(Sender: TObject;
  NewPage: TcxTabSheet; var AllowChange: Boolean);
begin
  AllowChange:= (DBMode=dmNone) or (DBMode=dmBrowse)
end;

procedure TSettingNeracaFrm.FormCloseQuery(Sender: TObject;
...
procedure TSettingNeracaFrm.actCloseExecute(Sender: TObject);
begin
  Close
end;

procedure TSettingNeracaFrm.actAddExecute(Sender: TObject);
begin
  try
    pgcMaster.ActivePageIndex:= 1;
    Master.Append;
    Detail.Close;
    Detail.Params.ParamByName('id_rek_neraca').Value:= '';
    Detail.Open;
...
procedure TSettingNeracaFrm.actEditExecute(Sender: TObject);
begin
  try
    pgcMaster.ActivePageIndex:= 1;
    Master.Edit;
...
procedure TSettingNeracaFrm.actDeleteExecute(Sender: TObject);
begin
...
procedure TSettingNeracaFrm.actRefreshExecute(Sender: TObject);
begin
  DBMode:= dmNone;
  try
    Master.Close;
...
procedure TSettingNeracaFrm.actSaveExecute(Sender: TObject);
begin
  DM.StartTransaction;
  try
...
procedure TSettingNeracaFrm.actCancelExecute(Sender: TObject);
begin
  try
    Master.Cancel;
...
procedure TSettingNeracaFrm.actCreateMenuExecute(Sender: TObject);
var f : TLookRekeningFrm;
begin

   try
     f:= TLookRekeningFrm.Create(Application);
     f.vid_rek:= Masterid_rek_neraca.AsString;
...
procedure TSettingNeracaFrm.grddbtvMasterCellDblClick(
...
procedure TSettingNeracaFrm.MasterNewRecord(DataSet: TDataSet);
begin
  Masterid_rek_neraca.Clear;
  Masterdiskripsi.Clear;
  Masterjenis.Clear;
  Masternperiode_ini.AsFloat:= 0;
  Masternperiode_lalu.AsFloat:= 0;
end;

procedure TSettingNeracaFrm.MasterBeforePost(DataSet: TDataSet);
begin
  if Masterid_rek_neraca.IsNull or (Trim(Masterid_rek_neraca.AsString)='') then
